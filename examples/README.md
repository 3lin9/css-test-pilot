...
`cases/order-create.yaml` 是 Phase 7 的目标场景:**小程序真实下单 → extract 订单号 → Web 后台 assert 该订单**。它验证 TestPilot 的核心价值:一套 DSL、两个 Adapter、变量跨端传递(含 locator 变量)。
...
# 3. testpilot.yaml 参考 examples/testpilot.yaml:
...
- cli 路径不需要配置:TestPilot 按 环境变量 `WECHAT_DEVTOOLS_CLI` → 常见安装位置 → 各盘符 Tencent 目录 → 注册表 的顺序自动探测
